...
*Setup instructions for a Gitian build of Mooncoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Mooncoin
...
- [Building Mooncoin Core](#building-mooncoin-core)
...
Clone the git repositories for mooncoin and Gitian.
...
in the mooncoin repository under 'Fetch and create inputs' to install sources which require
...
To build Mooncoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the mooncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mooncoin/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/mooncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit mooncoin=${COMMIT} --url mooncoin=${URL} ../mooncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mooncoin=${COMMIT} --url mooncoin=${URL} ../mooncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mooncoin=${COMMIT} --url mooncoin=${URL} ../mooncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mooncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mooncoin/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url mooncoin=${BTCPATH},signature=${SIGPATH} ../mooncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.moon` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mooncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mooncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mooncoin-osx-build.assert
```
